...
export class RecognizerWorker {
  ...
  private async createRecognizer({ recognizerId, sampleRate, grammar }: ClientMessageCreateRecognizer) {
    this.logger.verbose(
  ...
  private async processAudioChunk({ recognizerId, data, sampleRate }: ClientMessageAudioChunk) {
